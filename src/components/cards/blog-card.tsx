...
export const BlogCard = ({ blog }: BlogCardProps) => {
    return (
        <Link
            href={`/blog/${blog.id}`}
            className='flex flex-col rounded-3xl bg-border-gradient p-[0.125rem] shadow-blog-card'
        >
            <div className='flex grow flex-col rounded-3xl bg-blue'>
                <div className='relative flex overflow-hidden rounded-t-3xl'>
                    <Image
                        src={blog.image}
                        alt={blog.title}
                        className='w-full shrink-0 grow rounded-t-3xl object-cover duration-500 md:min-h-80'
                        width={500}
                        height={300}
                    />
                </div>
                <div className='flex grow flex-col justify-between gap-y-2 px-5 pb-5 pt-2'>
